...
cpp_src = "torch::Tensor mat_transpose_naive(torch::Tensor x);\
    \ntorch::Tensor mat_transpose_shared(torch::Tensor x);\
    \ntorch::Tensor mat_transpose_shared_pad(torch::Tensor x);"

funcs = ["mat_transpose_naive", "mat_transpose_shared", "mat_transpose_shared_pad"]
...
shape = 2**12
# 创建输入张量
x = torch.rand((shape, shape), dtype=torch.float32).cuda()

# 使用装饰器来 profile CUDA 核函数
torch.transpose = show_time(torch.transpose)
lib.mat_transpose_naive = show_time(lib.mat_transpose_naive)
lib.mat_transpose_shared = show_time(lib.mat_transpose_shared)
lib.mat_transpose_shared_pad = show_time(lib.mat_transpose_shared_pad)

# 计算正确结果
correct = torch.transpose(x, 0, 1)

# 计算 CUDA 核函数结果
cuda_res1 = lib.mat_transpose_naive(x)
cuda_res2 = lib.mat_transpose_shared(x)
cuda_res3 = lib.mat_transpose_shared_pad(x)
...
print("CUDA Transpose Result (first 10 elements):", cuda_res3[0][:2])
